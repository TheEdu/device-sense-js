...
/* NodeOPCUA Class*/
class NodeOPCUA {
  constructor(id = null, text = null, nodeClass = null, children = [], state = false, hidden = false, type = "root", parentName ="Root", dataType="NOTYPE") {
    this.id = id
    this.text = text
...
const getAddressSpace = (session, seedId, parentName, NodeCallback) => {
    ...
    callback => {
      session.readAllAttributes(seedId, function (err, res) {
        if (!err) {
          ...
          if (res.dataType) {
            nodeOPCUA.dataType = res.dataType.identifierType
          }
        }
        callback(err)
      })
    ...
    callback => {
      ...
      if (children.length == 0){
        ...
        async.each(children, function (child, refCallback) {
          getAddressSpace(session, child, nodeOPCUA.text, function (err, res){
            if(!err) {
  ...
  let addressSpace =  new Promise((resolve, reject) => {
    ...
    async.series([
        ...
        callback => {
            ...
            getAddressSpace(the_session, nodeId, "root", function (err,result){
                callback(err,result)
            })
        },
