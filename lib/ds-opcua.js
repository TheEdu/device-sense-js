...
class NodeOPCUA {
  constructor(id = null,
              ...
              type = "root",
              parentName = "Root",
              identifier = "NONE_IDENTIFIER",
              dataIdentifier = "NONE_DATA_IDENTIFIER",
              valueRank = -1) {
    this.id = id
    this.text = text
...
const getAddressSpace = (session, seedId, parentName, dataIdentifiers, NodeCallback) => {
  ...
  nodeOPCUA.parentName = parentName
  var children = []
  ...
  async.series([
    ...
    callback => {
      session.readAllAttributes(seedId, function (err, res) {
        if (!err) {
          ...
          if (res.nodeId) {
            nodeOPCUA.identifier = res.nodeId.value
          }
          if (res.dataType) {
            nodeOPCUA.dataIdentifier = res.dataType.value
          }
          nodeOPCUA.valueRank = res.valueRank
          // console.log(res)
        }
        callback(err)
      })
    ...
    callback => {
      ...
      if (children.length == 0){
        if (nodeOPCUA.nodeClass == 2 
          && (dataIdentifiers.includes(nodeOPCUA.dataIdentifier) || dataIdentifiers.length == 0)
          && nodeOPCUA.valueRank == -1)
        {
          nodeOPCUA.type = "child"
          nodeOPCUA.state.disabled = false
        } else {
          console.log(nodeOPCUA.dataIdentifier)
          nodeOPCUA.state.disabled = true
          nodeOPCUA.state.hidden = true
        }
        ...
        async.each(children, function (child, refCallback) {
          getAddressSpace(session, child, nodeOPCUA.text, dataIdentifiers, function (err, res){
            if(!err) {
...
/*******************************************************************************************/
exports.addressSpace = (endpointUrl, nodeId, timeout_ms, dataIdentifiers = []) => {
  // dataIdentifiers = []
  let addressSpace =  new Promise((resolve, reject) => {
    ...
    async.series([
        ...
        callback => {
            ...
            getAddressSpace(the_session, nodeId, "root", dataIdentifiers, function (err,result){
                callback(err,result)
            })
        },
